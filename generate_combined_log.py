...
for idx, w in enumerate(weeks_data):
    ...
    # Add a page break after each week except the very last one
    if idx < len(weeks_data) - 1:
        doc.add_page_break()

# ── Add Remaining Tasks Section at the end of the document ──
doc.add_page_break()
doc.add_heading('PROJECT OUTLOOK & REMAINING TASKS', 0)
doc.add_paragraph('While the frontend and UI/UX flows are 95% complete, there are a few critical milestones left before the final launch of ShopEase Nepal:')

doc.add_heading('1. Database Architecture & Setup', level=1)
doc.add_paragraph('Designing and provisioning the primary relational database (e.g., MySQL or PostgreSQL) to store users, products, orders, and wishlist data.', style='List Bullet')
doc.add_paragraph('Establishing secure connections and data models mapping directly to the frontend schemas.', style='List Bullet')

doc.add_heading('2. Backend REST API Integration', level=1)
doc.add_paragraph('Currently, the application relies on mock static data and local storage for authentication, cart, and product management. The next major phase will connect the frontend to a live backend database.', style='List Bullet')
doc.add_paragraph('Endpoints to integrate include: User Auth, Admin Product CRUD, Order Processing, and Wishlist syncing.', style='List Bullet')

doc.add_heading('3. Payment Gateway Setup', level=1)
doc.add_paragraph('Integrating Khalti and e-Sewa payment APIs to process real transactions during the checkout flow.', style='List Bullet')

doc.add_heading('4. Production Deployment', level=1)
doc.add_paragraph('Configuring GitHub Pages or a dedicated hosting environment (like Vercel/Netlify) for public access.', style='List Bullet')
doc.add_paragraph('Setting up automated CI/CD pipelines via GitHub Actions for continuous deployment.', style='List Bullet')

doc.save("WEEKLY_PROGRESS_LOG_ALL_WEEKS_FINAL_V2.docx")
print("Successfully created WEEKLY_PROGRESS_LOG_ALL_WEEKS_FINAL_V2.docx with remaining tasks and database setup.")
