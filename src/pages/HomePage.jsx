...
const HomePage = () => {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-105 font-sans selection:bg-amber-500 selection:text-white overflow-x-hidden transition-colors duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-6 space-y-6">
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
                ...
                <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">ShopEase Nepal</span>
                <span className="text-xs text-slate-300 dark:text-slate-700">|</span>
                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Home Delivery Across 7 Provinces</span>
              </div>
              ...
              <div className="w-full max-w-115 bg-white dark:bg-slate-950 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-2xl p-5 relative overflow-hidden">
                <div className="relative rounded-2xl overflow-hidden aspect-square shadow-inner">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 via-transparent to-transparent flex flex-col justify-end p-6">
                    ...
                    <p className="text-xs text-slate-200 dark:text-slate-300">Woven by hand in traditional wooden looms. 100% Cotton.</p>
                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/20">
                      <span className="text-lg font-extrabold text-white">Rs. {featuredProduct.price.toLocaleString()}</span>
                      <button 
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10">
            <div>
              ...
              <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight mt-1">Curated Local Collections</h2>
            </div>
            <button 
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {categories.map((cat) => (
              ...
              >
                ...
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition">{cat.name}</h3>
                <span className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold">{cat.count}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <section id="catalog" className="py-20 bg-slate-50/50 dark:bg-slate-900/20 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12 space-y-2">
            <span className="text-xs font-extrabold text-amber-600 uppercase tracking-widest block">Our Products</span>
            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Featured Nepalese Goods</h2>
          ...
          {filteredProducts.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-slate-950 rounded-2xl border border-slate-150 dark:border-slate-800 p-6">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Try resetting the filters or modifying your search query.</p>
              <button 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {filteredProducts.map((p) => (
                <Link key={p.id} to={`/product/${p.id}`} className="bg-white dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800 p-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group">
                  <div>
                    <div className="relative rounded-xl overflow-hidden aspect-square mb-4 bg-slate-100 dark:bg-slate-900">
                      <img
                        ...
                        loading="lazy"
                        onError={(e) => {
                          e.currentTarget.src = "https://images.unsplash.com/photo-1472851294608-062f824d29cc?w=600&auto=format&fit=crop&q=80"
                        }}
                      />
                      {p.badge && (
                    ...
                    <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 mt-1 mb-2 leading-snug group-hover:text-amber-600 dark:group-hover:text-amber-400 transition">{p.name}</h3>
                    <p className="text-xs text-slate-400 dark:text-slate-500 line-clamp-2 leading-relaxed mb-4">{p.description}</p>
                  </div>
                  
                  <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-50 dark:border-slate-800">
                    <span className="text-base font-extrabold text-slate-900 dark:text-white">Rs. {p.price.toLocaleString()}</span>
                    <button 
      ...
      <section id="delivery" className="py-20 bg-white dark:bg-slate-950 border-y border-slate-100 dark:border-slate-800 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-6">
          <NepalDeliveryMap
      ...
      <section className="py-20 bg-slate-50/50 dark:bg-slate-900/20 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16 space-y-2">
            ...
            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Customer Testimonials</h2>
            <p className="text-slate-400 dark:text-slate-500 text-sm font-semibold">Real feedback from shoppers across Nepal</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {testimonials.map((t) => (
              <div key={t.name} className="bg-white dark:bg-slate-950 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between hover:shadow-md hover:-translate-y-0.5 transition duration-200">
                <div>
      ...
      <section id="faq" className="py-20 bg-white dark:bg-slate-950 transition-colors duration-300">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="space-y-3.5">
            {faqs.map((faq) => (
              <div key={faq.q} className="bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden transition-all duration-200">
                <button 
                ...
                >
                  <span>{faq.q}</span>
                  <svg className={`w-4 h-4 text-slate-400 dark:text-slate-505 transition-transform duration-250 ${openFaq === faq.q ? "rotate-180 text-amber-600 dark:text-amber-400" : ""}`} fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {openFaq === faq.q && (
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          <div className="absolute inset-0 bg-slate-900/50 dark:bg-slate-950/70 backdrop-blur-sm transition-opacity" onClick={() => { if (!checkoutStep) setIsCartOpen(false); }} />
            ...
            <div className="w-screen max-w-md bg-white dark:bg-slate-955 border-l border-slate-100 dark:border-slate-800 shadow-2xl flex flex-col transform transition duration-300 animate-in slide-in-from-right duration-300">
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                {checkoutStep ? (
                  ...
                  <div className="text-center py-20 space-y-4">
                    ...
                    <p className="text-slate-400 dark:text-slate-505 text-xs max-w-[240px] mx-auto">Explore Nepalese crafts, garments, and tea to add them here.</p>
                  </div>
                ) : (
                  /* Item List */
                  <div className="space-y-4">
                    {cartItems.map((item) => (
                      <div key={item.id} className="flex gap-4 p-3 border border-slate-100 dark:border-slate-800 rounded-xl hover:shadow-sm transition">
                        <img src={item.image} alt={item.name} className="w-16 h-16 rounded-lg object-cover bg-slate-50 dark:bg-slate-900 shrink-0" />
                        
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 leading-snug line-clamp-1">{item.name}</h4>
              ...
              {checkoutStep && (
                <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 text-xs space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-550 dark:text-slate-400 font-medium">Shipping ({provincesData[province].name}):</span>
                    <span className="font-bold text-slate-700 dark:text-slate-300">Rs. {provincesData[province].shippingFee}</span>
                  </div>
                  <div className="flex justify-between border-t border-slate-200 dark:border-slate-800/80 pt-2 text-sm font-extrabold">
                    ...
                    <span className="text-amber-600 dark:text-amber-400 font-extrabold">Rs. {(cartSubtotal + provincesData[province].shippingFee).toLocaleString()}</span>
                  </div>
                </div>
              )}
      ...
      {orderSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-hidden">
          <div className="absolute inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm transition-opacity" onClick={() => setOrderSuccess(null)} />
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 border border-slate-100 dark:border-slate-800 shadow-2xl relative transform transition duration-300 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <h3 className="text-xl font-extrabold text-slate-900 dark:text-white text-center leading-tight">Order Placed Successfully!</h3>
            <p className="text-center text-xs text-slate-400 dark:text-slate-500 mt-1 font-semibold">
              Order ID: <span className="text-amber-600 dark:text-amber-400 font-bold">{orderSuccess.orderId}</span>
            </p>
            ...
            <div className="bg-slate-50 dark:bg-slate-950 rounded-2xl p-4.5 border border-slate-100 dark:border-slate-800 text-xs space-y-2 mt-5">
              ...
              <div className="pt-2 text-slate-600 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800">
                ...
                <span className="font-bold text-slate-700 dark:text-slate-300">{orderSuccess.address}, {orderSuccess.city}, {orderSuccess.provinceName}</span>
              </div>
            ...
            <div className="flex justify-between items-center text-sm font-bold pt-4 px-1 border-t border-slate-150 dark:border-slate-800 mt-5">
              ...
              <span className="text-amber-600 dark:text-amber-400 text-base font-extrabold">Rs. {orderSuccess.total.toLocaleString()}</span>
            </div>

            <p className="text-[10px] text-slate-450 dark:text-slate-500 leading-normal text-center mt-6">
              Our support desk will call you at <strong className="text-slate-605 dark:text-slate-300">{orderSuccess.phone}</strong> within 12 hours to verify your delivery address before dispatching the rider.
            </p>
