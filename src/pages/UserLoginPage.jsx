...
const UserLoginPage = () => {
  ...
  const { user, login, signup, verifyUserIdentity, userSetNewPassword,
          adminCredentials, error, setError, theme, loginWithGoogle } = useAuth();
            ...
            <div className="overflow-y-auto px-6 py-5 flex-1">
              <form onSubmit={handleSignup} autoComplete="off" className="space-y-4">
                ...
                <div className="relative flex items-center py-1">
                  ...
                  <span className="flex-shrink-0 mx-3 text-gray-400 dark:text-slate-500 text-xs">Or sign up with</span>
                  <div className="flex-grow border-t border-gray-200 dark:border-slate-800"></div>
                </div>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-center gap-3 py-2.5 rounded-xl font-semibold transition cursor-pointer border shadow-xs text-sm
                    ${dark 
                ...
                <p className={`text-center text-xs pt-1 pb-1 ${dark ? "text-slate-500" : "text-gray-600"}`}>
                  ...
                  <button type="button" onClick={closeSignup} className="text-amber-950 dark:text-amber-400 font-semibold hover:underline cursor-pointer">
                    Sign in instead
                  </button>
                </p>
              </form>
            ...
            <div className="w-full max-w-md">
              ...
              <div className="text-center mb-8">
                <div 
                  ...
                  className="mx-auto w-16 h-16 bg-gradient-to-br from-amber-950 to-orange-700 rounded-2xl flex items-center justify-center mb-4 shadow-lg shadow-amber-500/25 rotate-3 cursor-pointer select-none"
                >
              ...
              <form onSubmit={handleLogin} autoComplete="off" className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <button type="button" onClick={()=>setShowLoginPw(p=>!p)}
                          className={`absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer ${dark ? "text-slate-400 hover:text-amber-950" : "text-gray-600 hover:text-amber-950"}`}>
                          <EyeIcon show={showLoginPw} />
                        </button>
                      </div>
                      <div className="flex justify-end mt-1">
                        <button type="button" onClick={()=>{setRecoveryMode(true);setError("");}}
                          className="text-xs text-amber-900 hover:text-amber-950 hover:underline cursor-pointer">
                          Forgot Password?
                        </button>
                      </div>
                    </div>
                ...
                <div className="flex items-center">
                  <button type="button" onClick={openSignup}
                    className="flex items-center gap-1.5 text-sm text-amber-900 hover:text-amber-950 font-medium hover:underline cursor-pointer group">
                    <span className={`w-5 h-5 rounded-full flex items-center justify-center transition ${dark ? "bg-amber-900/40 group-hover:bg-amber-900/50" : "bg-amber-100"}`}>
                      <svg className="w-3 h-3 text-amber-950" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {!recoveryMode && (
                  <>
                    <div className="relative flex items-center py-2">
                      ...
                      <span className="flex-shrink-0 mx-4 text-gray-400 dark:text-slate-500 text-xs">Or continue with</span>
                      <div className="flex-grow border-t border-gray-300 dark:border-slate-700"></div>
                    </div>
                    
                    <button type="button" onClick={() => loginWithGoogle()}
                      className={`w-full flex items-center justify-center gap-3 py-3 rounded-xl font-semibold transition cursor-pointer border shadow-sm
                        ${dark 
                          ? "bg-slate-800 border-slate-700 text-white hover:bg-slate-700" 
                          : "bg-white border-gray-300 text-gray-700 hover:bg-gray-50"}`}>
                      <svg viewBox="0 0 24 24" className="w-5 h-5">
              ...
              <p className="text-center text-sm mt-6">
                <Link to="/" className="text-red-900 font-medium hover:text-amber-950 hover:underline">← Back to store</Link>
              </p>
            </div>
          </div>
        </div>
