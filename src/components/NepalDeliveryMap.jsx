...
const provincePaths = [
  { id: "sudurpashchim", d: "M 20 120 L 70 80 L 100 130 L 90 200 L 40 180 Z", label: "Sudurpashchim", x: 45, y: 150 },
...
const ProvinceDetails = ({ province }) => (
  <div className="bg-amber-50/50 dark:bg-amber-950/10 rounded-2xl border border-amber-100 dark:border-amber-900/30 p-5 shadow-sm space-y-3.5">
    <div className="flex items-center justify-between border-b border-amber-100/50 dark:border-amber-900/20 pb-2">
      <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-1.5">
        <span className="w-2 h-2 rounded-full bg-amber-500"></span>
        {province.name}
      </h4>
      <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 uppercase bg-white dark:bg-slate-900 px-2 py-0.5 rounded-full border border-amber-100 dark:border-amber-900/30">
        {province.status.includes("Express") ? "Express Available" : "Standard Active"}
      </span>
    </div>
    <p className="text-xs text-slate-650 dark:text-slate-400 leading-relaxed font-medium">{province.description}</p>
    <div className="grid grid-cols-2 gap-3 text-xs pt-1">
      <div>
        <span className="text-slate-400 dark:text-slate-500 block text-[9px] font-bold uppercase">Delivery Speed</span>
        <span className="font-bold text-slate-700 dark:text-slate-300">{province.deliveryTime}</span>
      </div>
      <div>
        <span className="text-slate-400 dark:text-slate-500 block text-[9px] font-bold uppercase">Shipping Fee</span>
        <span className="font-bold text-slate-700 dark:text-slate-300">Rs. {province.shippingFee}</span>
      </div>
    </div>
    <div>
      <span className="text-slate-400 dark:text-slate-500 block text-[9px] font-bold uppercase">Primary Logistics Hubs</span>
...
const NepalDeliveryMap = ({ selectedProvince, onSelectProvince, showDetails = true, compact = false }) => {
  ...
  const mapBlock = (
    <div className={`w-full bg-slate-50 dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-inner ${compact ? "" : "max-w-[620px]"}`}>
      <div className="text-center mb-4">
        <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 dark:text-slate-550 block">Nepal Delivery Map</span>
        <span className="text-xs font-bold text-slate-600 dark:text-slate-400">Click a province to check delivery info</span>
      </div>
      ...
      <svg viewBox="0 0 700 300" className="w-full h-auto drop-shadow-lg select-none" role="img" aria-label="Interactive map of Nepal provinces">
        {provincePaths.map((province) => (
          <path
            ...
            className={`stroke-white dark:stroke-slate-800 stroke-2 cursor-pointer transition-all duration-300 hover:fill-amber-400 ${
              selectedProvince === province.id ? "fill-amber-500" : "fill-slate-300 dark:fill-slate-700"
            }`}
          />
        ))}
        {provincePaths.map((province) => (
          <text key={`${province.id}-label`} x={province.x} y={province.y} className="text-[9px] font-bold fill-slate-700 dark:fill-slate-300 pointer-events-none">
            {province.label}
          </text>
        ))}
      </svg>

      <div className="flex items-center justify-center gap-4 mt-4 pt-4 border-t border-slate-200 dark:border-slate-800 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
